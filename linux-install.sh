#!/bin/bash
# Author: SAINT
# github: https://github.com/minerProxy-eth/lyminer

VERSION="2.8.6"

DOWNLOAD_HOST="https://github.com/minerProxy-eth/lyminer/raw/main/Linux-64"

DOWNLOAD_STANDBY="https://github.com/minerProxy-eth/lyminer/raw/main/Linux-64"

PATH_KT="/root/lymproxy"

PATH_EXEC="lyproxy"

PATH_CACHE="/root/lymproxy/.cache"

PATH_LICENSE="/root/lymproxy/license"

PATH_CONFIG="/root/lymproxy/.env"

PATH_NOHUP="/root/lymproxy/nohup.out"
PATH_ERR="/root/lymproxy/err.log"
...
turn_on() {
    ...
    if [ ! -f "$PATH_TURN_ON_SH" ];then
        ...
        echo '  echo "lyproxy已启动"' >> $PATH_TURN_ON_SH
        echo 'else' >> $PATH_TURN_ON_SH
        echo '  if [ $COUNT -ne 0 ]; then' >> $PATH_TURN_ON_SH
        echo '      echo "lyproxy已启动, 无需重复启动"' >> $PATH_TURN_ON_SH
        echo '  elif [ $(id -u) -ne 0 ]; then' >> $PATH_TURN_ON_SH
        echo '      echo "使用ROOT用户登录才能启动lyproxy"' >> $PATH_TURN_ON_SH
...
installapp() {
    ...
    colorEcho ${GREEN} "开始安装lyproxy-V-${VERSION}"
    ...
    checkProcess "lyproxy"
    if [ $? -eq 1 ]; then
        colorEcho ${RED} "发现正在运行的lyminer, 需要停止才可继续安装。"
        colorEcho ${YELLOW} "输入1停止正在运行的lyminer并且继续安装, 输入2取消安装。"
    ...
    wget -P $PATH_KT "${DOWNLOAD_HOST}/lyproxy_v${VERSION}_linux" -O "${PATH_KT}/${PATH_EXEC}" 1>/dev/null

    filterResult $? "拉取程序 lyproxy_v${VERSION}_linux"
...
check_hub() {
    ...
    tail -f /root/lymproxy/nohup.out
}

check_err() {
    colorEcho ${YELLOW} "按住CTRL+C后台运行"
    tail -f /root/lymproxy/err.log
}
...
echo "-------------------------------------------------------"
colorEcho ${GREEN} "欢迎使用lyminer安装工具, 请输入操作号继续。"
